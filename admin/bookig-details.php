...
<head>
	...
	<title>Fitness Assistance | New Bookings   </title>
...
<div id="print">
								<table border="1"  class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%"  >
									...
									<?php 
$bid=intval($_GET['bid']);
									$sql = "SELECT tblusers.*,tblbranches.BrandName,tbloffers.VehiclesTitle,tblbooking.FromDate,tblbooking.ToDate,tblbooking.message,tblbooking.VehicleId as vid,tblbooking.Status,tblbooking.PostingDate,tblbooking.id,tblbooking.BookingNumber,
DATEDIFF(tblbooking.ToDate,tblbooking.FromDate) as totalnodays,tbloffers.PricePerDay
									  from tblbooking join tbloffers on tbloffers.id=tblbooking.VehicleId join tblusers on tblusers.EmailId=tblbooking.userEmail join tblbranches on tbloffers.VehiclesBrand=tblbranches.id where tblbooking.id=:bid";
$query = $dbh -> prepare($sql);
	...
	<h3 style="text-align:center; color:red">#<?php echo htmlentities($result->BookingNumber);?> Booking Details </h3>
		...
		<tr>
										...
										<tr>
											...
											<tr>											
											<th>Offer Name</th>
											<td><a href="edit-offer.php?id=<?php echo htmlentities($result->vid);?>"><?php echo htmlentities($result->BrandName);?> , <?php echo htmlentities($result->VehiclesTitle);?></td>
											<th>Booking Date</th>
